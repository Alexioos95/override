...
A `level06` binary. Let's check the assembly:
	...
	0x080486b4 <+0>:     push   %ebp
	0x080486b5 <+1>:     mov    %esp,%ebp
	0x080486b7 <+3>:     sub    $0x18,%esp
	0x080486ba <+6>:     movb   $0x0,-0x9(%ebp)
	0x080486be <+10>:    jmp    0x80486c1 <clear_stdin+13>
	0x080486c0 <+12>:    nop
	0x080486c1 <+13>:    call   0x8048540 <getchar@plt>
	0x080486c6 <+18>:    mov    %al,-0x9(%ebp)
	0x080486c9 <+21>:    cmpb   $0xa,-0x9(%ebp)
	0x080486cd <+25>:    je     0x80486d5 <clear_stdin+33>
	0x080486cf <+27>:    cmpb   $0xff,-0x9(%ebp)
	0x080486d3 <+31>:    jne    0x80486c0 <clear_stdin+12>
	0x080486d5 <+33>:    leave
	0x080486d6 <+34>:    ret
	End of assembler dump.
	...
	0x080486e4 <+13>:    mov    0x804a080,%eax
	0x080486e9 <+18>:    mov    %eax,(%esp)
	0x080486ec <+21>:    call   0x8048530 <fflush@plt>
	0x080486f1 <+26>:    mov    $0x8048a60,%eax
	0x080486f6 <+31>:    lea    -0xc(%ebp),%edx
	0x080486f9 <+34>:    mov    %edx,0x4(%esp)
	0x080486fd <+38>:    mov    %eax,(%esp)
	0x08048700 <+41>:    call   0x80485e0 <__isoc99_scanf@plt>
	0x08048705 <+46>:    call   0x80486b4 <clear_stdin>
	0x0804870a <+51>:    mov    -0xc(%ebp),%eax
	0x0804870d <+54>:    leave
	0x0804870e <+55>:    ret
	End of assembler dump.
	...
	0x0804870f <+0>:     push   %ebp
	0x08048710 <+1>:     mov    %esp,%ebp
	0x08048712 <+3>:     mov    $0x1,%eax
	0x08048717 <+8>:     mov    $0x1,%ebx
	0x0804871c <+13>:    int    $0x80
	0x0804871e <+15>:    pop    %ebp
	0x0804871f <+16>:    ret
	End of assembler dump.

	(gdb) disas enable_timeout_cons
	Dump of assembler code for function enable_timeout_cons:
	0x08048720 <+0>:     push   %ebp
	0x08048721 <+1>:     mov    %esp,%ebp
	0x08048723 <+3>:     sub    $0x18,%esp
	0x08048726 <+6>:     movl   $0x804870f,0x4(%esp)
	0x0804872e <+14>:    movl   $0xe,(%esp)
	0x08048735 <+21>:    call   0x8048560 <signal@plt>
	0x0804873a <+26>:    movl   $0x3c,(%esp)
	0x08048741 <+33>:    call   0x8048570 <alarm@plt>
	0x08048746 <+38>:    leave
	0x08048747 <+39>:    ret
	End of assembler dump.
...
Input must have a length of atleast 5.
...
XOR encoding. Skipped if debug is detected.
...
Compare result of encoding with 2nd parameter.
...
Login system. Call `auth` with input 1 and 2.

So, we have to, somehow, get the right serial for our login. Let's use `gdb`, but we just can't simply follow the program continuously because of the `ptrace`.
	...
	***********************************
	...
	Breakpoint 2, 0x0804874e in auth ()

Now, we want to stop before the `ptrace` to let the var be initialized but not be detected yet.
...
Then, skip the if going into `ptrace`.

	(gdb) set $pc = 0x80487ed

Then, check the value in the stack.
